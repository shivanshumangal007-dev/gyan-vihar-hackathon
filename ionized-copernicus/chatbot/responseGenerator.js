 ...
 * Response Generator
 * 
 * Generates empathetic, brief, non-diagnostic responses based on:
 * - Emotion category
 * - Intensity level
 * 
 * SAFETY CONSTRAINTS:
 ...
 * - Never argues or invalidates
 * - Always brief and limited
 */
 ...
 */
function generateResponse(emotion, intensity) {
    // HIGH intensity always gets crisis response (emotion-agnostic)
    if (intensity === 'high') {
        return {
            emotion,
            intensity,
    ...
    // LOW or MEDIUM intensity - use emotion-specific templates
    const intensityTemplates = RESPONSE_TEMPLATES[intensity];

    if (!intensityTemplates) {
        // Fallback to low intensity unknown
        return {
            emotion,
            intensity,
